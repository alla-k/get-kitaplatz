package project.my.account.producer;

import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.kafka.core.KafkaTemplate;
import org.springframework.stereotype.Component;
import project.my.account.message.output.PlaceClaimedMessage;

@Component
@Slf4j
public class ReservationConfirmationProducer {
    private final KafkaTemplate<String, String> kafkaTemplate;
    private String topicName;

    @Value("${topic.output.place-claim}")
    public void setTopicName(String reservePlaceTopic) {
        this.topicName = reservePlaceTopic;
    }

    @Autowired
    public ReservationConfirmationProducer(KafkaTemplate<String, String> kafkaTemplate) {
        this.kafkaTemplate = kafkaTemplate;
    }

    public void sendMessage(PlaceClaimedMessage message) {
        log.info("Publish -> {}", topicName);
        kafkaTemplate.send(topicName, message.toJson());
    }

}
